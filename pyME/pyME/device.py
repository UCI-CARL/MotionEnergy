# this is all device code from the Motion Energy module
# it contains all __constant__ members and __global__ functions

from pycuda.compiler import SourceModule

mod = SourceModule("""

/* ********* MOTION ENERGY FUNCTIONS ********** */
...
__global__ void dev_filt2dir(float* d_respOut, float* d_respIn, unsigned int len, int nrXnrY, int nrScales, double speed=1.5) {
    const int     tid = IMUL(blockDim.x, blockIdx.x) + threadIdx.x;
    const int threadN = IMUL(blockDim.x, gridDim.x);
    ...
    for (unsigned int i=tid; i<len; i+=threadN) {
        ...
        int outInd = dir*nrXnrY+pos;

        d_respOut[outInd] += d_respIn[ind]*motionProj[speedInd][spaceTimeInd][dir];
    }
}
...
__global__ void dev_scale(float *data, double scale, int len) {
    const int     tid = IMUL(blockDim.x, blockIdx.x) + threadIdx.x;
    const int threadN = IMUL(blockDim.x, gridDim.x);
...
__global__ void dev_scaleHalfRect(float *data, int len, double scale, double spont) {
    const int     tid = IMUL(blockDim.x, blockIdx.x) + threadIdx.x;
    const int threadN = IMUL(blockDim.x, gridDim.x);
...
// copies src[i] to dest[i]
// this is basically memcpy DeviceToDevice, but it had to be re-implemented because array slicing
// with pycuda.driver.memcpy_dtod does not work in PyCUDA...
__global__ void dev_memcpy_dtod(float *dest, float *src, int len) {
    const int     tid = IMUL(blockDim.x, blockIdx.x) + threadIdx.x;
    const int threadN = IMUL(blockDim.x, gridDim.x);
...
__global__ void dev_split_gray(float *gray, unsigned char *idata, int len) {
    const int     tid = IMUL(blockDim.x, blockIdx.x) + threadIdx.x;
    const int threadN = IMUL(blockDim.x, gridDim.x);
